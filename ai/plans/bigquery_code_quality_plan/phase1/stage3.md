### 3. Implement `BigQueryExporter.insertData()` to handle basic data insertion into a BigQuery table. Focus on a simple schema first.
    ...
    insertData(tableName: string, data: unknown[]): boolean { // Applied lint fixes
        ...
        try {
            ...
            const rows = data.map(row => {
                return this.bigQueryService.newTableDataInsertAllRequestRows().json(row);
            });
...
- **Test Modification:** Add more unit tests to `bigquery_exporter_test.ts` for `insertData`, covering successful insertion of single/multiple rows and basic error cases.
  _ **Tool Call:**
  ```python
...
- **Run Tests (expecting all passing):**
  ...
  - **Tool Call:**
    ```python
    print(default_api.run_shell_command(command = "yarn test ts/common/tests/bigquery_exporter_test.ts", description = "Run BigQueryExporter data insertion unit tests"))
    ```
...
- **Git Commit:** `feat: Implement BigQueryExporter data insertion and pass unit tests`
  - **Tool Call:**
    ```python
    print(default_api.run_shell_command(command = "git add ts/common/bigquery_exporter.ts ts/common/tests/bigquery_exporter_test.ts && git commit -m "feat: Implement BigQueryExporter data insertion and pass unit tests"", description = "Commit BigQueryExporter data insertion and tests"))
    ```
  - **Expected Output:**
    ```bash
    [main <commit-hash>] feat: Implement BigQueryExporter data insertion and pass unit tests
