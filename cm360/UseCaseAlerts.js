...
function getGhostPlacementsAlerts(config, data) {
    ...
    data.forEach((row, index) => {
      ...
      if(advertiserName.startsWith('BidManager_Advertiser_DO_NOT_EDIT_') &&
      placementName.includes('DFA Zero placement')) {
        let brokenRules = `[${config.rules[0].ruleName}]`;
        const threshold = config.rules[2].ruleThreshold ? config.rules[2].ruleThreshold : 0; // Default to 0 if for some reason threshold is not provided
        ...
        issues.push(row);
      } else if (advertiserName.startsWith('BidManager_SeparateSpotlightAdvertiser_DO_NOT_EDIT') &&
  ...
  function getDefaultAdsRateAlerts(config, data) {
    ...
    for (let placementId in placementsMap) {
      ...
      const percentage = placement.sumImpressionsAllAds > 0 ? placement.sumImpressionsDefaultAds / placement.sumImpressionsAllAds : 0;
      if (placement.sumImpressionsAllAds === 0) {
        Logger.log(`Divisor is 0, setting percentage to 0 for Placement ID ${placementId}. sumImpressionsDefaultAds: ${placement.sumImpressionsDefaultAds} - sumImpressionsAllAds: ${placement.sumImpressionsAllAds}`);
      }
    ...
    data.forEach((row, index) => {
      ...
      const placementId = row[DA_PLACEMENT_ID_COLUMN - 1];
      const defaultAdsRatePercentage = placementsMap[placementId] ? placementsMap[placementId].defaultAdsPercentage : 0;
      const sumImpressionsDefaultAds = placementsMap[placementId] ? placementsMap[placementId].sumImpressionsDefaultAds : 0;
      const sumImpressionsAllAds = placementsMap[placementId] ? placementsMap[placementId].sumImpressionsAllAds : 0;
      // Rule evaluation happens here
      // Rule 1. Default Ads Rate > threshold - This rule is at position 0 in the rules array
      const threshold = config.rules[0].ruleThreshold ? config.rules[0].ruleThreshold : 0.02; // Default to 2% if for some reason threshold is not provided
      if (defaultAdsRatePercentage > threshold) {
        const pLabel = defaultAdsRatePercentage === 1 ? (defaultAdsRatePercentage * 100) : (defaultAdsRatePercentage * 100).toFixed(2);
  ...
  function getFloodlightTrendsAlerts(config, data) {
    ...
    data.forEach((row, index) => {
      ...
      const floodlightImpressions = parseInt(row[FT_FLOODLIGHT_IMPRESSIONS_COLUMN - 1]);
      ...
      switch(weekLabel) {
        case 'weekBeforeLast':
          if (!weekBeforeLastActivities[activityKey]) {
            ...
            console.log(`WARNING: Activity ${week} - ${activityKey} was found twice in the report for weekBeforeLastActivities`);
          }
          weekBeforeLastActivities[activityKey]['floodlightImpressionsSum'] += floodlightImpressions;
          break;
        case 'lastWeek':
          ...
          if (!lastWeekActivities[activityKey]) {
            ...
            console.log(`WARNING: Activity ${week} - ${activityKey} was found twice in the report for lastWeekActivities`);
          }
          lastWeekActivities[activityKey]['floodlightImpressionsSum'] += floodlightImpressions;
          break;
        case 'currentWeek':
          // Ignore Activity ${week} - ${activityKey} for the current week (Sunday) since they will be evaluated in the next run next week.`)
          break;
        default:
          Logger.log(`Activity ${week} - ${activityKey} is not from weekBeforeLast, lastWeek or currentWeek. Please check`);
      }
    ...
    data.forEach((row, index) => {
      ...
      switch(weekLabel) {
        case 'lastWeek':
          ...
          const weekBeforeLastFloodlightImpressionsSum = weekBeforeLastActivities[activityKey];
          if(weekBeforeLastFloodlightImpressionsSum) {
            ...
            ftEvaluateVarianceAndAddIssues(config, weekBeforeLastFloodlightImpressionsSum['floodlightImpressionsSum'], floodlightImpressions, row, issues);
          } else {
   ...
   */
  function ftEvaluateVarianceAndAddIssues(config, floodlightImpressionsWeekBefore, floodlightImpressionsWeek, row, issues) {
    const variance = (floodlightImpressionsWeekBefore - floodlightImpressionsWeek) / floodlightImpressionsWeekBefore;
    // Rule 1. For week on week trends flag for more > Threshold variance
    const threshold = config.rules[0].ruleThreshold ? config.rules[0].ruleThreshold : 0.1; // Default to 10% if for some reason threshold is not provided
    if (variance > threshold) {
      let brokenRules = `[${config.rules[0].ruleName}]`;
      row.push(config.rules[0].ruleType);
      const vLabel = variance === 1 ? (variance * 100) : (variance * 100).toFixed(2);
      row.push(`Rules: ${brokenRules}\n'Week before last' conversions for this Floodlight Configuration and Activity ID: ${floodlightImpressionsWeekBefore}\nVariance: ${vLabel}%`);
      issues.push(row);
    } else {
      // No issues, add an empty string
      row.push('');
  ...
  function getOutOfFlightPlacementsAlerts(config, data) {
    ...
    data.forEach((row, index) => {
      ...
      const threshold = config.rules[0].ruleThreshold ? config.rules[0].ruleThreshold : 1000; // Default to 1000 if for some reason threshold is not provided
      // If placementEndDate has ended and impressions > threshold
      if (today > placementEndDate && impressions > threshold) {
        let brokenRules = `Rules: [${config.rules[0].ruleName}]\nThreshold = ${threshold}`;
        row.push(config.rules[0].ruleType);
        row.push(brokenRules);
        issues.push(row);
  ...
  function getUseCasesConfiguration() {
    ...
    const useCases = useCasesSheet.getRange(USE_CASES_CONFIG_ROW_START, USE_CASES_CONFIG_COL_START, useCasesSheet.getLastRow(), useCasesSheet.getLastColumn()).getValues();
    let useCasesMap = {};
    useCases.forEach(useCaseRow => {
      ...
      const useCaseThreshold = parseFloat(useCaseRow[USE_CASES_CONFIG_THRESHOLD_COLUMN]);
      const otherConfigs = getOtherConfigsByUseCase(useCaseName, useCaseThreshold);
  ...
  function getOtherConfigsByUseCase(useCase, useCaseThreshold) {
    const config = {
      [GHOST_PLACEMENTS_KEY]: {
        ...
        'rangeToSort': 'A2:M',
        'rangeToFilter': 'A1:M',
        'sortByColumns': [{ 'column': GP_PLACEMENT_ID_COLUMN, 'ascending': true}, { 'column': GP_DATE_COLUMN, 'ascending': true }],
        'columnToResize': 13,
        'numColsToResize': 1
      },
      [DEFAULT_ADS_RATE_KEY]: {
        ...
        'rangeToSort': 'A2:O',
        'rangeToFilter': 'A1:O',
        'sortByColumns': [{ 'column': DA_PLACEMENT_ID_COLUMN, 'ascending': true}, { 'column': DA_DATE_COLUMN, 'ascending': true }],
        'columnToResize': 15,
        'numColsToResize': 1
      },
      [FLOODLIGHT_TRENDS_KEY]: {
        'rules': [{
          ruleName: 'For week on week trends flag for more than Threshold variance for floodlight impressions',
          ruleDescription: 'For week on week trends flag for more than Threshold variance for floodlight impressions',
          ruleType: 'ISSUE',
          ruleThreshold: useCaseThreshold,
          color: '#ea4335'
        }],
        ...
        'rangeToSort': 'A2:H',
        'rangeToFilter': 'A1:H',
        'sortByColumns': [{ 'column': FT_WEEK_COLUMN, 'ascending': false}, { 'column': FT_ACTIVITY_ID_COLUMN, 'ascending': true }],
        'columnToResize': 8,
        'numColsToResize': 1
      },
      [OUT_OF_FLIGHT_PLACEMENTS_KEY]: {
        'rules': [{
          ruleName: 'Placements with impressions > Threshold out of flight schedule',
          ruleDescription: 'Placements with impressions > Threshold out of flight schedule',
          ruleType: 'ISSUE',
          ruleThreshold: useCaseThreshold,
          color: '#ea4335'
        }],
        ...
        'rangeToSort': 'A2:N',
        'rangeToFilter': 'A1:N',
        'sortByColumns': [{ 'column': OFP_PLACEMENT_ID_COLUMN, 'ascending': true}, { column: OFP_DATE_COLUMN, ascending: true }],
        'columnToResize': 14,
        'numColsToResize': 1
      },
      [TRACKING_ADS_KEY]: {
        ...
        'rangeToSort': 'A2:L',
        'rangeToFilter': 'A1:L',
        'sortByColumns': [{ 'column': TA_PLACEMENT_ID_COLUMN, 'ascending': true}, { 'column': TA_DATE_COLUMN, 'ascending': true }], //TODO: CHANGE THIS!!!!
        'columnToResize': 12,
        'numColsToResize': 1
      }
    }
