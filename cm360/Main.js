...
function main() {
  ...
  try {
    ...
    const reports = reportsSheet.getRange(REPORTS_CONFIG_ROW_START, REPORTS_CONFIG_COL_START, reportsSheet.getLastRow(), reportsSheet.getLastColumn()).getValues();
    ...
    reports.forEach((reportRow, row) => {
      ...
      if (!profileId) {
        Logger.log(`WARNING: Profile ID not provided. Skipping this row ${row}`);
        if (useCase) {
          logExecutionStatus(row, `WARNING: Profile ID not provided. Skipping this row.`, `${date.toDateString()} ${date.toTimeString()}`);
        }
        return
      }
      ...
      const accounts = campaignManagerAPI.list('Accounts', null, 'accounts', {});
      ...
      if (useCase && !enabled) {
        ...
        logExecutionStatus(row, `Use Case ${useCase} is disabled for Profile ID ${profileId} and CM360 Account ${accountId}. Execution skipped.`, `${date.toDateString()} ${date.toTimeString()}`);
        return
      }
      ...
      if (!reportId) {
        Logger.log(`Report ID not provided for Profile ID ${profileId} and CM360 Account ${accountId}, creating a new report in CM360...`);
        const dateRange = reportRow[REPORTS_CONFIG_REPORT_DATE_RANGE - 1];
        const filters = reportRow[REPORTS_CONFIG_FILTERS_COLUMN - 1];
        const extraParams = reportRow[REPORTS_CONFIG_EXTRA_PARAMS_COLUMN - 1].toString();
        if(useCase === FLOODLIGHT_TRENDS_KEY && !extraParams) {
          logExecutionStatus(row, `ERROR: The Extra Parameters column is required when executing the ${FLOODLIGHT_TRENDS_KEY} use case. Execution skipped.`, `${date.toDateString()} ${date.toTimeString()}`);
          return
        }
        if(useCase === FLOODLIGHT_TRENDS_KEY && dateRange != 'LAST_14_DAYS') {
          logExecutionStatus(row, `ERROR: Date range for the ${FLOODLIGHT_TRENDS_KEY} use case must be LAST_14_DAYS. Execution skipped.`, `${date.toDateString()} ${date.toTimeString()}`);
          return
        }
        const report = campaignManagerAPI.createAndRunReport(useCase, dateRange, filters.trim(), extraParams.trim());
        reportId = report.id;
        // Add report Id and account Id to the Config sheet for visualization purposes and future executions of existing reports
        spreadsheet.getSheetByName(REPORTS_CONFIG_SHEET_NAME).getRange(row + 2, REPORTS_CONFIG_ACCOUNT_ID_COLUMN).setValue(accountId);
        spreadsheet.getSheetByName(REPORTS_CONFIG_SHEET_NAME).getRange(row + 2, REPORTS_CONFIG_REPORT_ID_COLUMN).setValue(reportId);
      }

      // Get data from the existing report
      let reportData = getExistingReportData(useCase, profileId, accountId, reportId, campaignManagerAPI);
      if (!reportData || reportData.length <= 1) {
        Logger.log('main: There is no data in the report. Skipping execution...');
        // Long message... format later
        logExecutionStatus(row, 'WARNING: The report is not ready or there is no data in the report. Please check the report status directly in CM360 and run again the script or wait for the trigger to run it automatically. Execution was skipped.', `${date.toDateString()} ${date.toTimeString()}`);
        return
      }
      ...
      addDataToSheet(reportDataSheetName, BLUE_RGB_COLOR, dataToSheetStartRow, dataToSheetStartColumn, config.columnToResize, config.numColsToResize, reportData);
      flagUseCaseIssues(config, reportDataSheetName);
      // Sort table by the columns that make sense for the use case
      shortTableByColumns(reportDataSheetName, config.rangeToSort, config.sortByColumns);
      ...
      if(emails && issues['issues'].length > 0) {
        ...
        sendEmail(getEmailParameters(useCase, profileId, accountId, reportId, emailMessage, emails));
      } else {
        Logger.log(`There are no issues identified for use case ${useCase} or email recipients were not provided for alerts. Skipping email notification...`);
      }

      // Add execution timestamp and status
      logExecutionStatus(row, `SUCCESS`, `${date.toDateString()} ${date.toTimeString()}`);
      
    });
    
  } catch(e) {
    Logger.log(`ERROR: ${e}`);
    logExecutionStatus(currentRow, `ERROR: ${e}`, `${date.toDateString()} ${date.toTimeString()}`);
  }
}
 ...
 */
function getExistingReportData(useCase, profileId, accountId, reportId, campaignManagerAPI) {
  const latestReportFile = campaignManagerAPI.getLatestReportFile(reportId);
  if (!latestReportFile) {
    Logger.log(`The report for Profile ID ${profileId} and CM360 Account ID ${accountId} is still running for use case ${useCase} and it will probably take longer than the script execution limit (5 mins) in this Google Spreadsheet.`);
    return
  }
  const latestReportFileData = campaignManagerAPI.getLatestReportFileDataByRedirectURL(latestReportFile);
...
function flagUseCaseIssues(config, reportDataSheetName) {
  addConditionalFormattingToSheet(reportDataSheetName, config.flagColumnRange, config.rules);
}