 ...
 * 
 */
function addDataToSheet(sheetName, tabColor, startRow, startColumn, columnToResize, numColsToResize, data) {
    if(data.length === 0) {
  ...
  function clearSheetRange(sheetName, startRow, startColumn) {
    let sheet = spreadsheet.getSheetByName(sheetName);
    sheet.getRange(startRow, startColumn, sheet.getDataRange().getLastRow(), sheet.getDataRange().getLastColumn()).clear();
  }
